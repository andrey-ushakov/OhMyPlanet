...
void GestureRecognition::recognizeGesture() {
  ...
  if(_isNoGesture) {  // after each recognized gesture player has to return to initial state
    if(z < -55) {
      _isNoGesture = false;
...
bool GestureRecognition::isComboAvailaible() {
  return _comboLength == MAX_COMBO_LENGTH;
}
